...
working
    ...
    works with 2v2 and 3 v3
        need to do the state stuff
            hp normalized
            with the new revealed or not
            and th

        add stuff for num pkm on each side

        2v3 and 3v3
        need sub directory for savings based on tag then num pkm
        args for num pkm
        make sure the code works
        make sure the state is correct
        what size state? shoudl I collapse 2v3 and 3v3 down?
        how to handle the reveal rolls
    ...
    if 10k in a state and at elast 1k of each action, then can continue

    fix my sim with dmg to actually calculate the dmg turns to faint accurately
        make the ttf standard hp
        make the hp standard hp offset for agent pkm
...
def build_train_eval_loop(
    ...
    for battle_idx in range(num_battles):
        ...
        for episode_step in range(max_episode_steps):
            ...
            # get state key
            state_list_agent = get_turns_to_faint_list(
                game_state[0],
                game_state[1],
                max_turns_to_faint_value,
                is_get_hp_normalizer=True,
            ...
            state_list_opp = get_turns_to_faint_list(
                game_state[1],
                game_state[0],
                max_turns_to_faint_value,
                is_get_hp_normalizer=False,
                is_reveal_team_1_active_moves=is_reveal_opp_active_moves,
                is_reveal_team_1_party_0_moves=is_reveal_opp_party_0_moves,
                is_reveal_team_1_party_moves=is_reveal_opp_party_1_moves,
            ...
            if terminated:
                ...
                if is_3v3_battle:
                    ...
                    action_state_results_dict = add_results_state_list_to_action_dict(
                        ...
                        agent_first_move_attack_key, agent_first_move_swap_party_0_key, agent_first_move_swap_party_1_key)
